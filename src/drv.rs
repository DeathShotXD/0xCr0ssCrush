...
impl CrossDev {
    ...
    pub fn kill_pid(&self, kind: Kind, pid: u32) -> Result<(), String> {
        ...
        // Alinubx also reads a DWORD exit status at +4; leave 0 for a
        // normal termination. DCRC only reads the PID DWORD; extra bytes
        // are ignored (METHOD_BUFFERED input length check is >= 4).
        let mut out = [0u8; 4];
        let mut ret = 0u32;
        unsafe {
            DeviceIoControl(
