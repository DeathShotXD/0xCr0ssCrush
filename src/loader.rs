...
/// SCM lifecycle for loading rspot.sys as a kernel service.
/// Random name is chosen by the caller; the service is stopped and
/// deleted on drop so no artifact survives the run.
