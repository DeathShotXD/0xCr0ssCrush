...
use windows::Win32::Foundation::{CloseHandle, GENERIC_WRITE, WIN32_ERROR};
...
pub fn random_service_name(prefix: &str) -> String {
...
// Point stdin/stdout/stderr at the NUL device (GUI-subsystem blends).
pub unsafe fn silence_std_handles() -> Result<(), windows::core::Error> {
    ...
    use windows::Win32::System::Console::{SetStdHandle, STD_ERROR_HANDLE, STD_INPUT_HANDLE, STD_OUTPUT_HANDLE};
    let nul_name = "NUL";
    let wstr: Vec<u16> = nul_name.encode_utf16().chain(Some(0)).collect();
    let nul = CreateFileW(
        PCWSTR(wstr.as_ptr()),
        (windows::Win32::Foundation::GENERIC_READ.0 | windows::Win32::Foundation::GENERIC_WRITE.0) as u32,
